...
const CATS = ["Produce","Dairy","Meat","Bakery","Frozen","Beverages","Pantry"];
...
const SUPPLIERS_SEED = [
  { id:"sup1", name:"FreshFarm Co.",   contact:"orders@freshfarm.com",   phone:"(212)555-0101", leadDays:2, rating:4.8, categories:["Produce","Dairy"],     active:true },
  { id:"sup2", name:"MeatWorks LLC",   contact:"supply@meatworks.com",   phone:"(213)555-0202", leadDays:1, rating:4.6, categories:["Meat"],                active:true },
  { id:"sup3", name:"BakeryDirect",    contact:"hello@bakerydirect.com", phone:"(312)555-0303", leadDays:1, rating:4.9, categories:["Bakery"],              active:true },
  { id:"sup4", name:"ColdChain Dist.", contact:"ops@coldchain.com",      phone:"(415)555-0404", leadDays:3, rating:4.5, categories:["Frozen","Beverages"],  active:true },
  { id:"sup5", name:"Pantry Plus",     contact:"bulk@pantryplus.com",    phone:"(718)555-0505", leadDays:4, rating:4.3, categories:["Pantry"],              active:true },
];
const USERS_SEED = [
...
function makeProds(sid){
  return [
    {id:`${sid}-1`,storeId:sid,name:"Organic Whole Milk",    category:"Dairy",   sku:"DAI-001",barcode:"012345678901",price:3.49, stock:42,minStock:15,expiryDays:7,  addedDate:daysAgo(2),description:"Fresh organic whole milk, 1 gallon",  tag:{synced:true, lastSync:"2m ago"}, supplierId:"sup1"},
    {id:`${sid}-2`,storeId:sid,name:"Sourdough Bread",       category:"Bakery",  sku:"BAK-007",barcode:"098765432109",price:5.99, stock:18,minStock:10,expiryDays:4,  addedDate:daysAgo(1),description:"Artisan sourdough loaf, 800g",        tag:{synced:true, lastSync:"5m ago"}, supplierId:"sup3"},
    {id:`${sid}-3`,storeId:sid,name:"Atlantic Salmon Fillet",category:"Meat",    sku:"MEA-023",barcode:"111222333444",price:12.99,stock:8, minStock:12,expiryDays:2,  addedDate:daysAgo(3),description:"Fresh Atlantic salmon, per lb",       tag:{synced:false,lastSync:"2h ago"},supplierId:"sup2"},
    {id:`${sid}-4`,storeId:sid,name:"Roma Tomatoes",         category:"Produce", sku:"PRO-041",barcode:"444333222111",price:1.99, stock:65,minStock:20,expiryDays:5,  addedDate:daysAgo(1),description:"Fresh Roma tomatoes, per lb",         tag:{synced:true, lastSync:"1m ago"}, supplierId:"sup1"},
    {id:`${sid}-5`,storeId:sid,name:"Greek Yogurt Plain",    category:"Dairy",   sku:"DAI-012",barcode:"555666777888",price:4.29, stock:30,minStock:20,expiryDays:14, addedDate:daysAgo(0),description:"Full-fat Greek yogurt, 32oz",        tag:{synced:true, lastSync:"8m ago"}, supplierId:"sup1"},
    {id:`${sid}-6`,storeId:sid,name:"Frozen Peas",           category:"Frozen",  sku:"FRZ-003",barcode:"123456789012",price:2.49, stock:55,minStock:25,expiryDays:365,addedDate:daysAgo(10),description:"Sweet green peas, frozen, 16oz",   tag:{synced:false,lastSync:"1d ago"}, supplierId:"sup4"},
    {id:`${sid}-7`,storeId:sid,name:"Orange Juice",          category:"Beverages",sku:"BEV-019",barcode:"888777666555",price:3.79,stock:28,minStock:15,expiryDays:10, addedDate:daysAgo(3),description:"100% fresh squeezed OJ, 64oz",      tag:{synced:true, lastSync:"3m ago"}, supplierId:"sup4"},
    {id:`${sid}-8`,storeId:sid,name:"Free Range Eggs",       category:"Dairy",   sku:"DAI-005",barcode:"210987654321",price:6.49, stock:5, minStock:10,expiryDays:21, addedDate:daysAgo(5),description:"Cage-free large brown eggs, 12ct", tag:{synced:true, lastSync:"11m ago"},supplierId:"sup1"},
  ];
}
const SEED_ORDERS = [
  {id:"ORD-1001",storeId:"s1",supplierId:"sup1",supplierName:"FreshFarm Co.",status:"delivered",placedAt:daysAgo(5),eta:"3 days ago",total:284.50,items:[{name:"Organic Whole Milk",qty:40,unitPrice:3.49},{name:"Greek Yogurt Plain",qty:30,unitPrice:4.29}],notes:"Urgent restock",priority:"high"},
  {id:"ORD-1002",storeId:"s1",supplierId:"sup2",supplierName:"MeatWorks LLC",status:"in-transit",placedAt:daysAgo(2),eta:"Tomorrow",total:519.60,items:[{name:"Atlantic Salmon Fillet",qty:40,unitPrice:12.99}],notes:"",priority:"normal"},
...
const CATALOG_SEED = [
  ...
  { id:"cat-6", supplierId:"sup2", name:"Atlantic Salmon Fillet", category:"Meat",     sku:"SUP2-MEA-023", barcode:"111222333444", unitPrice:8.50, packSize:"per lb",    minOrderQty:5,  maxOrderQty:200, leadDays:1, expiryDays:2,   description:"Fresh Atlantic salmon, skin-on fillet.",     status:"active",  stock:80,  images:["🐟"] },
  { id:"cat-7", supplierId:"sup2", name:"Chicken Breast",         category:"Meat",     sku:"SUP2-MEA-031", barcode:"222333444555", unitPrice:4.20, packSize:"per lb",    minOrderQty:10, maxOrderQty:500, leadDays:1, expiryDays:3,   description:"Boneless skinless chicken breast, fresh.",   status:"active",  stock:240, images:["🍗"] },
  ...
  { id:"cat-11",supplierId:"sup4", name:"Orange Juice",           category:"Beverages",sku:"SUP4-BEV-019", barcode:"888777666555", unitPrice:2.40, packSize:"64oz",      minOrderQty:8,  maxOrderQty:300, leadDays:3, expiryDays:10,  description:"100% fresh-squeezed orange juice.",          status:"active",  stock:175, images:["🍊"] },
  { id:"cat-12",supplierId:"sup4", name:"Frozen Peas",            category:"Frozen",   sku:"SUP4-FRZ-003", barcode:"123456789012", unitPrice:1.50, packSize:"16oz",      minOrderQty:12, maxOrderQty:600, leadDays:3, expiryDays:365, description:"Flash-frozen sweet garden peas.",            status:"active",  stock:430, images:["🫛"] },
  { id:"cat-13",supplierId:"sup4", name:"Sparkling Water 12pk",   category:"Beverages",sku:"SUP4-BEV-033", barcode:"456456456456", unitPrice:5.20, packSize:"12 cans",   minOrderQty:4,  maxOrderQty:200, leadDays:3, expiryDays:365, description:"Lightly sparkling natural mineral water.",   status:"active",  stock:290, images:["💧"] },
  { id:"cat-14",supplierId:"sup5", name:"Extra Virgin Olive Oil", category:"Pantry",   sku:"SUP5-PAN-007", barcode:"789789789789", unitPrice:6.80, packSize:"500ml",     minOrderQty:6,  maxOrderQty:200, leadDays:4, expiryDays:730, description:"Cold-pressed first extraction EVOO.",        status:"active",  stock:120, images:["🫒"] },
  { id:"cat-15",supplierId:"sup5", name:"Jasmine Rice 5lb",       category:"Pantry",   sku:"SUP5-PAN-021", barcode:"654654654654", unitPrice:3.90, packSize:"5lb bag",   minOrderQty:6,  maxOrderQty:400, leadDays:4, expiryDays:365, description:"Fragrant long-grain jasmine rice.",          status:"active",  stock:310, images:["🌾"] },
  { id:"cat-16",supplierId:"sup1", name:"Cheddar Cheese Block",   category:"Dairy",    sku:"SUP1-DAI-022", barcode:"987987987987", unitPrice:3.50, packSize:"8oz block", minOrderQty:8,  maxOrderQty:250, leadDays:2, expiryDays:60,  description:"Sharp aged cheddar, natural rind.",          status:"draft",   stock:200, images:["🧀"] },
];

/* ═══════════════════════════════════════════════════════════ SUPPLIER PORTAL */
function SupplierPortal({session,stores,allOrders,setAllOrders,toast,logout}){
                    ...
                    <div key={o.id} className="card" style={{padding:20,borderLeft:`3px solid ${ORDER_STATUS_COLOR[o.status]}`}}>
                      ...
                      {o.status!=="delivered"&&o.status!=="cancelled"&&(
                        <div style={{display:"flex",gap:7,marginTop:14,flexWrap:"wrap"}}>
                          ...
                          {o.status==="processing"&&<button className="bp" style={{padding:"6px 14px",fontSize:12,background:C.amber}} onClick={()=>updateOrderStatus(o.id,"in-transit")}>Mark Dispatched</button>}
            ...
            <div key={p.id} className="card" style={{padding:0,overflow:"hidden",cursor:"pointer",transition:"border-color .2s",borderColor:p.status==="draft"?C.border2:C.border}} onClick={()=>setShowDetail(p)}>
              ...
              <div style={{padding:"14px 16px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div style={{fontSize:28}}>{p.images?.[0]||"📦"}</div>
...
function CatalogProductModal({ product, onSave, onClose }){
  ...
  const [form, setForm] = useState(product ? { ...product } : {
    ...
    description:"", status:"draft", images:["📦"],
  });
    ...
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" style={{width:580}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,color:C.muted,marginBottom:6}}>Product icon</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {EMOJI_OPTIONS.map(e=>(
              <button key={e} onClick={()=>set("images",[e])} style={{fontSize:20,padding:"6px 10px",borderRadius:8,border:`1px solid ${form.images?.[0]===e?C.green:C.border2}`,background:form.images?.[0]===e?`${C.green}18`:"none",cursor:"pointer"}}>
                {e}
              </button>
            ))}
          </div>
...
/* ═══════════════════════════════════════════════════════════ STORE DASHBOARD */
function StoreDashboard({session,stores,suppliers,allProducts,setAllProducts,allOrders,setAllOrders,syncingIds,syncTag,toast,logout}){
  ...
  function receiveOrder(id){
    ...
    order.items?.forEach(item=>{
      ...
      if(prod) setAllProducts(all=>all.map(p=>p.id===prod.id?{...p,stock:p.stock+parseInt(item.qty||0)}:p));
    });
  ...
  const TABS=["dashboard","inventory","price-tags","orders","analytics","alerts"];
              ...
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:860}}>
                ...
                <tbody>
                  {filtered.map(p=>{
                    ...
                    return(
                      <tr key={p.id} className="trow" style={{background:"transparent"}}>
                        <td><div style={{fontWeight:500}}>{p.name}</div><div style={{fontSize:11,color:C.muted}}>{p.sku}</div></td>
